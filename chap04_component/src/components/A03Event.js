...
function A03Event() {
  ...
  const changeString = (evt) => {
    ...
    if (evt.target.value.trim().length <= 10) {      // 10글자까지만 입력
  ...
  const changeLanguage = (evt) => {
    ...
    if (!data.language.includes(value)) {
      ...
      const newLanguage = data.language.concat(value);  // 새로운 language
      const newData = { ...data, language: newLanguage };  // language도 새롭게, data도 새로운 값으로
      setData(newData);
    } else {
      // 이미 선택 되어있던 값 삭제 해제 시킨것
  ...
  return (
    <div className="mb-5">
      ...
      <form>
        ...
        <div className="mb-3">
          <label htmlFor="name" className="form-label">Address: {data.address}</label>
