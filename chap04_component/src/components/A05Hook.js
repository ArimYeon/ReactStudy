...
function A05Hook() {
  ...
  */
  ...
  const changeNumber = useCallback((evt) => {
    setData((x) => {
      // x는 현재 setData의 getter값이 주입된다. 즉 data의 현재 값 => {num: 10, str: ''}
      let value = Number(evt.target.value);
      if (Number.isNaN(value)) value = '';
  ...
  // 의존관계에 상태변수, 훅을 사용한 함수가 지정된 경우 => 상태변수, 함수가 변경되는 경우만 재 실행된다
  useEffect(() => {
    setTimeout(() => {
      setToday(new Date())
    }, 2000);
    ...
    // 변수 정리 등..
    // 언마운트 시점에 실행 => useEffect 실행 => 클린어 함수 실행 = useEffect 실행
  ...
  }, [data.list]);  // data.list가 변경되는 경우에만 이 함수를 재계산


  // 5. useRef: 값을 유지할 목적. useState와는 달리 값이 변경되도 리렌더링 하지 않는다
