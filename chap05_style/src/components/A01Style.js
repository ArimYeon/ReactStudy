...
// sass 라이브러리 설치 후 반드시 프로젝트 재시작 해야한다
import './css/A01Style.scss';

function A01Style() {
  const myStyle = { backgroundColor: 'lightgray', color: 'white', fronSize: '20px', padding: '10px' };
  const [style, setStyle] = useState({ backgroundColor: 'lightgray', color: 'white', fronSize: '20px', padding: '10px' });
  ...
  return (
    <div className="mb-5">
      {/* style 정의. HTML 방식의 style은 에러. 반드시 { key: value, ..} 형태여야 한다 */}
      <h3 style={{ backgroundColor: 'lightgray', color: 'white', fronSize: '20px', padding: '10px' }}>A01 Style</h3>
